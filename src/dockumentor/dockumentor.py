...
def render_template(template_name, output_path, context):
    ...
    # Navigate up two directories from the current file and then to the 'templates' directory
    # Check if the template file exists
    if "dockumentor_compose.md" in template_name:
        template_dir = os.path.join(os.path.dirname(__file__), 'templates')
        template_path = os.path.join(template_dir, template_name)

        if not os.path.exists(template_path):
            print(f"Template not found: {template_name}. Looking in directory: {template_path}")
            raise FileNotFoundError("Template not found.")
        
        print(f"Template directory: {template_dir}")  # Debugging output to check the path
...
def parse_ports(ports):
    """Parse and format port configurations to handle multiple formats, including IP:port:port."""
    parsed_ports = []
    for port in ports:
        parts = port.split(':')
...
def generate_sankey_diagram_network(services, networks):
    ...
    for service_name, details in services.items():
        parsed_ports = parse_ports(details['ports'])
        ...
        if parsed_ports:
            for port in parsed_ports:
                if "127.0.0.1" in port:
                    # Internal port binding (accessible only within the host)
                    port_details = port.replace("127.0.0.1", "Internal")
...
def generate_sankey_diagram_depends(services, networks):
    ...
    connections = set()  # Use a set to avoid duplicates
    for service_name, details in services.items():
        service_ports = parse_ports(details.get('ports', []))
        ...
        if depends_on_services:
            for dependency in depends_on_services:
                if isinstance(dependency, dict):  # Handling cases where depends_on could be a YAML reference
                    ...
                    dependency_ports = parse_ports(services[dependency_name].get('ports', []))
                else:
                    dependency_name = dependency
                    dependency_ports = parse_ports(services[dependency].get('ports', []))
...
# Integration with existing code in the document_docker_compose function:
def document_docker_compose(compose_file_path, template_path, output_path):
    ...
    for service_name, service_details in compose_data.get('services', {}).items():
        services[service_name] = {
            'ports': parse_ports(service_details.get('ports', [])),
    ...
    render_template(template_path if template_path else 'dockumentor_compose.md', output_path, context)
...
def cli(compose_file, template, output):
    """CLI to generate documentation from a docker-compose file."""
    document_docker_compose(compose_file, template, output)
